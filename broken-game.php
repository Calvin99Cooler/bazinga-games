<?php
// File to store broken game reports
$file = "broken-game.txt";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    } elseif (strlen($gameName) > 30) {
        $error = "The game name must be 30 characters or less.";
    } else {
        $timestamp = date("m/d/y - H:i");
        $line = $timestamp . " | " . $gameName . PHP_EOL;
        ...
        <head>
            ...
            <style>
                *{margin:0;padding:0;box-sizing:border-box;font-family:'Poppins',sans-serif}
                ...
                .message h2{font-size:1.5rem;background:linear-gradient(45deg,#fff,#aaa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:1rem}
                .message p{color:#ccc}
            </style>
        </head>
        <body>
...
<title>Report Broken Game</title>
<style>
*{margin:0;padding:0;box-sizing:border-box;font-family:'Poppins',sans-serif}
...
input[type="text"]{width:100%;padding:.75rem 1rem;margin:8px 0;border-radius:25px;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.05);color:#fff;outline:none;font-size:1rem}
...
<form action="" method="POST">
    ...
    <input type="text" id="game_name" name="game_name" maxlength="30" required>
